...
export function startBossBattle(){
  ...
  const baseHp = 3000;
  const hpMultiplier = game.bossCount > 0 ? (2 ** (game.bossCount - 1)) : 1;
  const currentMaxHp = baseHp * hpMultiplier;
...
export function updateBoss(dt) {
  ...
  if (boss.shootTimer <= 0) {
    ...
    const baseBulletSpeedMin = 150;
    const baseBulletSpeedMax = 250;
    const speedMultiplier = game.bossCount > 0 ? (1 + 0.2 * (game.bossCount - 1)) : 1;

    for (let i = 0; i < bulletCount; i++) {
      const angle = startAngle + (Math.PI * 2 / bulletCount) * i + rand(-0.05, 0.05); // 약간의 무작위성
      const spd = rand(baseBulletSpeedMin, baseBulletSpeedMax) * speedMultiplier; // 속도 랜덤화 및 보스 카운트에 따른 증가
...
export function updateBossBullets(dt) {
  for (let i = game.bossBullets.length - 1; i >= 0; i--) {
    ...
    if (game.player.invuln <= 0 && circleHit(b, game.player)) {
      const baseDamage = 20;
      const damageMultiplier = game.bossCount > 0 ? (1 + 0.3 * (game.bossCount - 1)) : 1;
      game.player.hp -= baseDamage * damageMultiplier;
