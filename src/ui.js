...
export function maybeLevelUp(){
  ...
  while(game.player.xp >= game.player.xpNext){
    ...
    // 레벨 5 도달 시 보스전 시작 (레벨업 로직 안에 두는 것이 적절)
    if (game.player.level === 5 && !game.isBossBattle) {
      startBossBattle();
    }
  }
...
export function openLevelUp(){
  ...
  const pool = [
    ...
    {name:"쿨다운 -10%", apply:()=>game.player.cooldown*=0.9, desc:"자동 사격 주기 10% 단축"},
    ...
    {name:"이동속도 +10%", apply:()=>game.player.speed*=1.1, desc:"기본 이동속도 10% 증가"},
